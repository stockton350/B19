...
import { initTTS, speak, stopSpeaking, isTTSReady, VOICES, warmAudioContext, setTTSApiKey } from './tts.js';
...
const STORAGE = { KEY: 'b19_key', PERSONA: 'b19_persona', VOICE: 'b19_voice', MODE: 'b19_mode' };
const BAR_COUNT = 28;
...
const cfg = {
  ...
  voice:   localStorage.getItem(STORAGE.VOICE)   || 'Aoede',
...
function onInit() {
  ...
  if (!key) { flash('api-key', 'ENTER YOUR API KEY'); return; }
  cfg.apiKey  = key;
    ...
    onResult: async transcript => {
      ...
      try {
        ...
        await speak(reply, cfg.voice, null, () => {
          if (cfg.mode === 'AUTO') {
            setTimeout(startSession, 500); // auto-restart listening after response
          } else {
            transitionTo(S.IDLE);
          }
        });
